...
function update(){
  ...
  let line='ちょうどいい厚さの毛布だね',face='happy';
  if(a>80){line='これはもう金星…!暑い、重い、つぶれる〜!';face='hot';}
  else if(heat>.5){line='毛布が厚すぎるよ…熱がこもって出ていかない…';face='hot';}
  else if(heat>.15){line='ちょっと蒸してきたかも…';face='hot';}
  else if(a<0.02){line='空気がなさすぎて、昼は焼けて夜は凍っちゃう…息もできない…';face='cold';}
  else if(ice>.3){line='毛布がうすい…さむい…';face='cold';}
  speechLine.textContent=line;
  setFace(face);
  if(a>50)whyAtm.textContent='🔎 金星(大気92倍相当)の実測は約460℃・92気圧。このざっくりモデルでも90倍で約480℃。いい線いってるでしょ';
  else if(a>5)whyAtm.textContent='🔎 大気が濃いほど温室効果の毛布が厚くなり、熱がこもるよ(今は+'+deltaT.toFixed(0)+'℃の上乗せ)';
  else if(a>=0.3)whyAtm.textContent='🔎 地球の温室効果は約+33℃。この毛布がなければ平均気温は−18℃の氷の星だよ';
  else if(a>=0.02)whyAtm.textContent='🔎 大気1%未満の火星は温室効果がほぼ働かず、平均−63℃。毛布の大切さがわかるね';
